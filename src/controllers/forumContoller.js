...
const create = async (req, res) => {
  const item = await Forum.create(req.body);
  sendSuccess(res, item, 'Created successfully', 201);
};

const update = async (req, res) => {
  const item = await Forum.findByIdAndUpdate(req.params.id, req.body, { returnDocument: 'after' });
  if (!item) return sendError(res, 'Not found', 404);
  sendSuccess(res, item, 'Updated successfully');
};

const remove = async (req, res) => {
  const item = await Forum.findByIdAndDelete(req.params.id);
  if (!item) return sendError(res, 'Not found', 404);
